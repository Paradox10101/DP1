// public/workers/animation-worker.js

// Definir la configuración localmente
const ANIMATION_CONFIG = {
  ...
  function getOrCreateVehicleState(vehicleCode, position) {
    if (!vehicleStates.has(vehicleCode)) {
      vehicleStates.set(vehicleCode, new VehicleAnimationState(position));
    }
    return vehicleStates.get(vehicleCode);
  }
  ...
  self.onmessage = function(e) {
    ...
    const updatedFeatures = batch.map(feature => {
      ...
      const state = getOrCreateVehicleState(feature.properties.vehicleCode, position);
      
      const updated = state.updatePosition(position, deltaTime, {
